...
import { useLazyGetAllRatingReviewsQuery, useLazyGetRatingStarListQuery } from '../../redux/features/ratingSliceAPI';
...
function HomeItemDetail() {
    ...
    // RTK Quries
    const [allRatingReviewTrigger, allRatingReviewResult] = useLazyGetAllRatingReviewsQuery();
    const [ratingStarListTrigger, ratingStarListResult] = useLazyGetRatingStarListQuery();
    const [subCategoryItemTrigger, subCategoryItemResult] = useLazyGetSubCategoryItemQuery();
    ...
    useEffect(() => {
        if (itemIdString) {
            ...
            const res = allRatingReviewTrigger({ itemId, page, size }).then((resp) => {
                if (resp.isSuccess) {
    ...
    const handleViewMore = async () => {
        ...
        setPage(newPage);
        await allRatingReviewTrigger({ itemId, page: newPage, size }).then((resp) => {
            if (resp.isSuccess) {
        ...
        <div className="bg-background px-4 pt-6 sm:px-5 sm:pt-4 xl:px-7">
            ...
            <div className="grid grid-cols-12 gap-8 xl:gap-4">
                ...
                <div className="rounded-xl bg-white p-3 shadow-md xl:col-span-5 2xl:col-span-4">
                    <HomeItemDetailAccordin data={subCategoryItemData?.data.homeCatItemFaq} />
                </div>
            </div>
                    ...
                    <div className="2xl:col-span-2 xl:col-span-3">
                            ...
                            {total !== 0 ?
                                <>
                                    <span className="text-4xl font-semibold">
                                        {(Number(ratingStarListData?.data?.total) / total).toFixed(2)}
                                    </span>
                                    <div className="mx-4 flex items-center rounded-full bg-black px-4 text-white">
                                        ...
                                        <span className="mx-2 text-sm">
                                            {handleRatingText(Number(ratingStarListData?.data?.total / allRatingReviewData?.data?.total))}
                                        </span>
                                    </div>
                                </>
                                : null}
                        ...
                        <div className="mt-2">
                            <Rating
                                ...
                                value={handleRatingValue(Number(ratingStarListData?.data?.total / allRatingReviewData?.data?.total))}
                                precision={0.5}
                                readOnly
                            />
                        ...
                        <div className="text-[#6A6A6A]">{allRatingReviewData?.data?.total} Reviews</div>
                    </div>
                    <div className="2xl:col-col-span-10 xl:col-span-9 border-l-[1px]">
                        <div>
                                ...
                                ?.map((ratings: any, index: number) => {
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="2xl:col-span-3 xl:col-span-6 mx-10">
